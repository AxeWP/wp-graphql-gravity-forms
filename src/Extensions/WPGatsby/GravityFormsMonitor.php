...
if ( class_exists( 'WPGatsby' ) ) {
	...
	class GravityFormsMonitor extends \WPGatsby\ActionMonitor\Monitors\Monitor {
		 ...
		 */
		public function __construct( \WPGatsby\ActionMonitor\ActionMonitor $action_monitor ) {
		...
		public function init() {
			 ...
			 * Save draft entry.
			 *
			 * @todo Enable once draft entries are part of the schema.
			 */
			/* add_action( 'gform_incomplete_submission_post_save', [ $this, 'after_save_draft_entry' ], 10, 2 );*/ //phpcs:ignore
		}
		...
		public function after_create_form( int $form_id ) : void {
			$args = [
				...
				'title'               => "Form #{$form_id}",
				'node_id'             => $form_id,
				'relay_id'            => Relay::toGlobalId( Form::$type, (string) $form_id ),
		...
		public function after_create_entry( array $entry ) : void {
			$args = [
				...
				'node_id'             => $entry['id'],
				'relay_id'            => Relay::toGlobalId( Entry::$type, (string) $entry['id'] ),
				'graphql_single_name' => Entry::$type,
				'graphql_plural_name' => Entry::$type . 's',
				// Forms don't have post status. This is for Gatsby.
				'status'              => 'publish',
			];
		...
		public function after_update_entry( array $form, int $entry_id ) : void {
			$args = [
				...
				'node_id'             => $entry_id,
				'relay_id'            => Relay::toGlobalId( Entry::$type, (string) $entry_id ),
				'graphql_single_name' => Entry::$type,
				'graphql_plural_name' => Entry::$type . 's',
				// Forms don't have post status. This is for Gatsby.
				'status'              => 'publish',
		...
		public function after_save_draft_entry( array $submission, string $resume_token ) : void {
			$args = [
				...
				'relay_id'            => Relay::toGlobalId( Entry::$type, $resume_token ),
				'graphql_single_name' => Entry::$type,
				'graphql_plural_name' => Entry::$type . 's',
				// Forms don't have post status. This is for Gatsby.
				'status'              => 'publish',
